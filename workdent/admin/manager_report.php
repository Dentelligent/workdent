...
<div id="main">
 ...
 <div class="container-fluid  custom-container">
       ...
       $sql = "SELECT * from managers  WHERE ExecutiveName = '$name' ORDER BY AppointmentDateTime DESC";
        ...
        if ($result->num_rows > 0) {
            ...
            while($row = $result->fetch_assoc()) {
                ...
                foreach ($photoNames as $photoName) {
                    ...
                    $imageLinks .= "<a href='../manager/uploads/" . $photoName . "' target='_blank'>Image " . $imageNumber . "</a><br>";
                    $imageNumber++; // Increment image number
                }
              
               echo "<tr><td>" . $row["id"]. "</td><td>" . $formattedDate. "</td><td>" . $row["ClinicName"]. "</td><td>" . $row["DoctorName"]. "</td><td>" . $row["Area"]. "</td><td>" . $row["Service"].  "</td><td>" . $row["Caseid"]. "</td><td>" . $row["ExecutiveName"]. "</td><td>" . $row["Remarks"]. "</td><td><a href='https://maps.google.com/maps?q=" . $row['Latitude'] . "," . $row['Longitude'] . "&hl=es&z=14' target='_blank'> Location URL</a></td><td>" .  $imageLinks. "</td></tr>";
